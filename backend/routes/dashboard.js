...
router.get('/', async (req, res) => {
  try {
    ...
    const [emps, facturesMois, reglesMois, impayesData, docsExp, contratsRenouv] = await Promise.all([
      ...
      supabase.from('factures').select('total_ttc,statut').eq('camping_id', cid).eq('periode', mois),
      supabase.from('reglements').select('mode,montant,date_reglement').eq('camping_id', cid).gte('date_reglement', `${mois}-01`),
      listImpayes(cid),
